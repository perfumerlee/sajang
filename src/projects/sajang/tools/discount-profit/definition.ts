...
export const discountProfitTool: ToolDefinition = {
  ...
  knowledge: {
    ...
    example: { title: '정상 판매가격 20,000원, 현재 판매량 100개, 할인율 10%', inputs: exampleInput, result: { discountPrice: formatWon(exampleResult.discountPrice), discountUnitContribution: formatWon(exampleResult.discountUnitContribution), requiredQuantity: formatQuantity(exampleResult.requiredQuantity), requiredIncreaseRate: formatPercent(exampleResult.requiredIncreaseRate) }, explanation: `production calculator 결과 할인 판매가격 ${formatWon(exampleResult.discountPrice)}, 필요한 판매량 ${formatQuantity(exampleResult.requiredQuantity)}, 판매량 증가 필요율 ${formatPercent(exampleResult.requiredIncreaseRate)}입니다.` },
    limitations: ['사장도구의 계산 결과는 사용자가 입력한 조건을 기준으로 한 단순 추정입니다. 실제 사업 결과는 세금, 부가가치세, 금융비용, 환불, 폐기, 할인, 계절성 및 기타 비용 등에 따라 달라질 수 있습니다.', '할인 후 실제 판매량은 고객 반응, 상품 특성, 경쟁가격, 시장 수요 등에 따라 달라질 수 있습니다.', '이 계산기는 수요 예측 모델이 아니라 할인으로 감소한 개당 기여금을 보전하기 위해 필요한 판매량을 계산하는 의사결정 지원 도구입니다.'],
